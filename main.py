...
def main(argv=None):
    ...
    if dataset_name == 'random':
        ...
        dataset = datasets.RandomGraphDataset(nb_nodes=1000, nb_edges=2000, nb_examples=10000,
        ...
        transform_adj_func = None if not_norm_adj else datasets.ApprNormalizeLaplacian(compute_path)
    ...
    optimizer = torch.optim.SGD(cgn.parameters(), lr=learning_rate, momentum=momentum, weight_decay=weight_decay)
    ...
    for t in range(epoch):
        ...
        for my_set, set_name in zip([train_set, valid_set, test_set], ['train', 'valid', 'test']):
            acc[set_name] = accuracy(my_set, cgn)
