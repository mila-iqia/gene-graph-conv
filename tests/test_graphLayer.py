...
class TestMaxPooling(unittest.TestCase):
    ...
    def testIdToKeep(self):
        ...
        x = np.arange(self.nb_nodes * 2 * 3) .reshape((3, 2, self.nb_nodes))# Our value, 2 channel, 3 examples.
        to_keep = np.arange(0, self.nb_nodes) % 2  # We keep one node on two.
        solution = np.zeros((self.nb_nodes,2))
        ...
        np.testing.assert_almost_equal(proposed[0].numpy().reshape(-1), solution.reshape(-1)) # Only test the first example.
