...
class MLMethods(Method):
    ...
    def __init__(self, column_names, model_name="GCN", num_epochs=100, num_channel=16, num_layer=2, add_emb=8, use_gate=False, dropout=False, cuda=False, seed=0, adj=None, graph_name=None):
    ...
    def fit(self, X, y):
        ...
        if self.model_name == "GCN":
            adj_transform, aggregate_function = models.graphLayer.get_transform(adj, self.cuda, num_layer=self.num_layer)
        ...
        for epoch in range(0, self.num_epochs):
            for base_x in range(0, X_train.shape[0], self.batch_size):
                ...
                loss = optimization.compute_loss(criterion, y_pred, labels)
