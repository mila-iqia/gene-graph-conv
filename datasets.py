...
class Graph(object):
    def __init__(opt):
        adj = None
        if opt.scale_free:
            load_random_adjacency(nb_nodes=100, approx_nb_edges=100, scale_free=opt.scale_free)
        else:
            load_graph(opt.graph_path)
        self.nb_nodes = self.adj.shape[0]

    @classmethod
    def load_random_adjacency(nb_nodes, approx_nb_edges, scale_free=True):
...
class GraphDataset(Dataset):
    def __init__(self, name, opt, graph):
        ...
        self.seed = opt.seed
        self.nb_class = opt.nb_class

        self.nb_examples = self.data.size()[0] if opt.nb_examples is None else opt.nb_examples
        if opt.center:
...
class GraphGeneDataset(GraphDataset):
    ...
    def __init__(self, data_dir=None, data_file=None, nb_class=None, sub_class=None, graph=None, **kwargs):
        """
        Args:
            data_file (string): Path to the h5df file.
        """
...
class RandomGraphDataset(GraphDataset):
    ...
    def load_data(self):
        ...
        # Generating the data
        self.data = np.random.randn(opt.nb_examples, self.nb_nodes, 1)
...
class GBMDataset(GraphGeneDataset):
    ...
    def __init__(self, data_dir="/data/lisa/data/genomics/TCGA/", data_file="gbm.hdf5", nb_class=2, **kwargs):
        super(GBMDataset, self).__init__(data_dir=data_dir, data_file=data_file, nb_class=nb_class, name='GBMDataset', **kwargs)
