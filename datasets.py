...
def split_dataset(dataset, batch_size=100, random=False, validate=True, train_ratio=0.8, seed=1993):

    nb_example = len(dataset)
    all_idx = range(nb_example)
    ...
    train_set = DataLoader(dataset, batch_size=batch_size, sampler=SubsetRandomSampler(all_idx[:nb_train]))

    if validate:

        nb_rest = (nb_example - nb_train)/2
        test_set = DataLoader(dataset, batch_size=batch_size, sampler=SubsetRandomSampler(all_idx[nb_train:nb_train + nb_rest]))
        valid_set = DataLoader(dataset, batch_size=batch_size, sampler=SubsetRandomSampler(all_idx[nb_train+nb_rest:]))

        return train_set, valid_set, test_set

    else:

        test_set = DataLoader(dataset, batch_size=batch_size, sampler=SubsetRandomSampler(all_idx[nb_train:]))
